...
Publisher.prototype.publish = function (headers, options) {
  ...
  return through.obj(function (file, enc, cb) {
    ...
    if (file.isBuffer()) {
      ...
      // calculate etag
      etag = '"' + file.bucket + ':' + md5Hash(file.contents) + '"';
	  ...
	  // add gzip header
	  if (file.gzip) file.s3.headers['Content-Encoding'] = 'gzip';

      // add extra headers
      for (header in headers) file.s3.headers[header] = headers[header];

	  // For HTML, override the cache-control header
	  if (options.noCacheForHtml) {
		var ext = (file.unzipPath || file.path).replace(/.*[\.\/\\]/, '').toLowerCase();
		if ('html' === ext) {
      ...
      // get s3 headers
	  var bucket = file.bucket || _this.config.params.Bucket;
      _this.client.headObject({ Bucket: bucket, Key: file.s3.path }, function(err, res) {
        ...
        // update: file are different
          ...
          file.s3.state = (res.ETag ? 'update' : 'create') + ' (' + bucket + ')';
